...
export default function Navbar() {
  ...
  const REQUIRED    = 8   // 필요 클릭 수
  const INTERVAL_MS = 800 // 연속 인식 간격

  const handleLogoClick = (e: React.MouseEvent) => {
    // 기본 Link 동작(홈 이동) 막고 카운터 처리
    e.preventDefault()
    ...
    if (clickCount.current >= REQUIRED) {
      clickCount.current = 0
      if (clickTimer.current) clearTimeout(clickTimer.current)
      router.push('/admin')
    } else if (clickCount.current === 1) {
      // 1번째 클릭은 홈으로 이동 (일반 사용자 경험 유지)
      router.push('/')
    }
  }
